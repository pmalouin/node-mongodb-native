...
var Pool = function(topology, options) {
  ...
  // there is always a monitoring socket, so we need to bump the `minSize` by one
  this.options.minSize = this.options.minSize + 1;
...
function resetPoolState(pool, options) {
  options = options || {};
...
Pool.prototype.connect = function(callback) {
  ...
  createConnection(this, (err, conn) => {
    ...
    // mark the first connection for monitoring
    conn[kMonitor] = true;
 ...
 * @method
 */
Pool.prototype.destroy = function(options, callback) {
  const self = this;
  ...
  if (force) {
    const forceError = options.forceError || new MongoError('Pool was force destroyed');
    ...
    while (self.queue.length > 0) {
      ...
      if (typeof workItem.cb === 'function') {
        workItem.cb(forceError);
      }
    }

    // Destroy the topology
    return destroy(self, connections, options, callback);
  }
 ...
 */
Pool.prototype.reset = function(options, callback) {
  ...
  // destroy existing connections which have no work items
  const connections = this.availableConnections
    .concat(this.inUseConnections)
    .filter(conn => conn.workItems.length === 0);
  ...
  eachAsync(
    ...
    err => {
      ...
      resetPoolState(this, { skipConnections: true });
      this.inUseConnections = this.inUseConnections.filter(conn => conn.workItems.length !== 0);
      ...
      createConnection(this, (err, conn) => {
        if (conn) {
          // mark the first connection the monitor connection
          conn[kMonitor] = true;
        }
...
function _execute(self) {
  return function() {
    ...
    while (true) {
      ...
      let connection;
      const nonMonitoringConnections = self.availableConnections.filter(
        conn => conn[kMonitor] == null
      );
      ...
      if (workItem.monitoring) {
        connection = self.availableConnections.find(conn => conn[kMonitor] === true);
        if (connection == null) {
          self.queue.unshift(workItem);
          break;
        }
      } else {
        connection = nonMonitoringConnections.filter(conn => conn.workItems.length === 0)[0];
        if (connection == null) {
          if (nonMonitoringConnections.length === 0) {
            self.queue.unshift(workItem);
            createConnection(self);
            break;
          }

          // choose a random connection for "pipelining"
          connection =
            nonMonitoringConnections[self.connectionIndex++ % nonMonitoringConnections.length];
        }
      }
