...
function monitorServer(server, options) {
  ...
  checkServer((err, isMaster) => {
    ...
    server.s.pool.reset({ forceError: err }, resetError => {
      if (resetError) {
        // unable to create a new monitoring socket
        server.emit(
          'descriptionReceived',
          new ServerDescription(server.description.address, null, { error: err })
        );
      ...
      checkServer((error, isMaster) => {
        if (error) {
          ...
          // NOTE: we use the _first_ error encountered here
          server.emit(
            'descriptionReceived',
            new ServerDescription(server.description.address, null, { error: err })
          );
